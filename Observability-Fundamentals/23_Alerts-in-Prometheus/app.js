...
const httpFailureMetric = new promClient.Counter({
  name: "http_request_failure_counter",
  help: "This will count how many request has been failed",
  labelNames: ["method", "path", "status"],
});

// to collect histogram metric
...
app.use((req, res, next) => {
  ...
  const stopHttpReq = httpReqDuration.startTimer({ method, path });

  res.on("finish", () => {
    gaugeMetrics.labels(method, path).dec();
    httpsRequestTotal.labels(method, path, res.statusCode).inc();

    // to stop collecting histogram metric
    stopHttpReq({ status: res.statusCode });
  });

  if (req.statusCode >= 400) {
    httpFailureMetric.labels(method, path, res.statusCode).inc();
  }
  ...
  req.on("aborted", cleanGauge);
  req.on("close", cleanGauge);
  req.on("error", cleanGauge);
...
app.post("/alert", (req, res, next) => {
  ...
  res.json({ data: "you got the data" });
});
...
// http error counter metric : which will track all https errors
